        ...
        <section id="pricing" className="py-32 px-6 md:px-16 bg-background">
            <div className="max-w-7xl mx-auto">
                <div className="text-center mb-20">
                    ...
                    <p className="font-serif italic text-2xl text-dark/80">
                ...
                <div className="grid md:grid-cols-3 gap-8 items-center max-w-6xl mx-auto relative">
                    {tiers.map((tier, idx) => (
                        <div
                            key={idx}
                            className={`rounded-[3rem] p-10 flex flex-col h-full ${tier.highlight
                                ? 'bg-primary text-background shadow-xl transform md:scale-105 z-10'
                                : 'bg-white text-dark border hover:shadow-lg transition-shadow'
                                }`}
                        >
                            <div className="mb-8">
                                {tier.highlight && (
                                ...
                                <p className={`text-sm ${tier.highlight ? 'text-background/90' : 'text-dark/70'}`}>{tier.desc}</p>
                            </div>

                            <div className="mb-10 flex items-baseline gap-2">
                                <span className="font-sans font-bold text-5xl">{tier.price}</span>
                                <span className={`text-sm font-mono ${tier.highlight ? 'text-background/80' : 'text-dark/60'}`}>/{tier.period}</span>
                            </div>
                            ...
                            <button
                                className={`w-full py-4 rounded-full font-semibold magnetic-transform transition-colors ${tier.highlight
                                    ? 'bg-accent text-dark hover:bg-dark hover:text-background'
