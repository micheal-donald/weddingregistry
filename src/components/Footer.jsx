...
export default function Footer() {
  ...
  return (
    <footer className="bg-dark text-background rounded-t-[4rem] pt-16 pb-12 px-6 md:px-16 mt-[-4rem] relative z-20">
      <div className="max-w-7xl mx-auto">
        {/* Minimal Header */}
        <div className="flex justify-between items-center">
          <h3 className="font-serif italic text-4xl font-bold text-accent">Gifted</h3>
          <button
            onClick={() => setIsExpanded(!isExpanded)}
            className="flex items-center gap-2 font-mono text-xs uppercase tracking-widest text-background/80 hover:text-accent transition-colors border border-background/20 rounded-full px-6 py-2.5 hover:bg-background/5"
          >
            {isExpanded ? 'Hide Menu' : 'Explore'}
            <svg viewBox="0 0 24 24" fill="none" className={`w-4 h-4 transition-transform duration-500 ${isExpanded ? 'rotate-180' : ''}`} stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
        ...
        {/* Expandable Grid */}
        <div className={`w-full overflow-hidden transition-all duration-700 ease-in-out ${isExpanded ? 'max-h-[1000px] opacity-100 mt-12 mb-16' : 'max-h-0 opacity-0 mt-0 mb-8'}`}>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 md:gap-8 pt-12 border-t border-background/10">
            ...
            <div className="md:col-span-1 flex flex-col items-start md:items-end">
              <Link to="/register" className="px-8 py-4 bg-accent text-dark rounded-full font-semibold magnetic-transform hover:scale-105 transition-all mb-8 shadow-lg shadow-accent/20 text-center">
