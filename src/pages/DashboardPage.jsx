...
export default function DashboardPage() {
  ...
  const fetchRegistries = () => {
    ...
    getMyRegistries()
      .then(setRegistries)
      .catch((err) => setError(err.message || 'Failed to load registries'))
      ...
      <div className="min-h-screen bg-background">
        <div className="bg-white/80 border-b border-dark/5 px-6 py-5 animate-pulse">
          <div className="max-w-6xl mx-auto flex items-center justify-between">
            <div className="h-8 w-32 bg-dark/5 rounded" />
            <div className="h-8 w-24 bg-dark/5 rounded-full" />
          </div>
        </div>
        <div className="max-w-6xl mx-auto px-6 py-12">
          <div className="h-8 w-48 bg-dark/5 rounded mb-8 animate-pulse" />
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="bg-white rounded-[2rem] shadow-lg p-6 border border-dark/5 animate-pulse">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-10 h-10 rounded-full bg-dark/5" />
                  <div className="space-y-2 flex-1">
                    <div className="h-4 w-3/4 bg-dark/5 rounded" />
                    <div className="h-3 w-1/2 bg-dark/5 rounded" />
                  </div>
                </div>
                <div className="h-3 w-1/3 bg-dark/5 rounded mb-4" />
                <div className="flex items-center justify-between pt-4 border-t border-dark/5">
                  <div className="h-5 w-16 bg-dark/5 rounded-full" />
                  <div className="h-4 w-20 bg-dark/5 rounded" />
                </div>
              </div>
            ))}
          </div>
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={fetchRegistries} className="bg-primary text-background px-6 py-2.5 rounded-full font-semibold hover:shadow-lg transition-all">
            Try Again
          </button>
        </div>
      </div>
